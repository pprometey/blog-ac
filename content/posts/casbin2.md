...
## Описание задачи
У нас имеется CMS, в которую пользователи могут добавлять стати. Пользователь может видеть только свои статьи и имеет роль `user`. Статьи других людей он не может видеть, если только ему не присвоена роль `admin` или `supervisor`. `supervisor` может видеть и редактировать все статьи, а `admin` имеет все те же права что и `supervisor`, но кроме этого, еще может и удалять любые статьи.
